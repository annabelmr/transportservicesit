...
public class ThreadingBatchingListProcessor {
    ...
    public static <T> long process(final String logNodeName, final int batchSize,
        ...
        final ILogNode log = Core.getLogger(logNodeName);
        ...
        do {
            ...
            if (count > 0) {
                ...
                log.info(String.format("Processed %d items (total: %d).", count, total));
            }
        } while (count == batchSize);

        if (total > 0) {
            log.info(String.format("Done, processed all items (took: %d ms, total: %d).",
